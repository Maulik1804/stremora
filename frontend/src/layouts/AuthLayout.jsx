...
const AuthLayout = () => {
  ...
  return (
    <div className="min-h-screen auth-gradient flex items-stretch">

      {/* ── Left panel — sticky so it stays in view while right side scrolls ── */}
      <div className="hidden lg:flex lg:w-[44%] xl:w-[42%] flex-shrink-0 sticky top-0 self-start"
           style={{ height: '100vh' }}>
        <div className="flex flex-col justify-between w-full p-12 relative overflow-hidden border-r border-white/5">
          {/* Ambient glow */}
          ...
          {/* Logo */}
          <Link to="/" className="flex items-center select-none relative z-10">
            <img src="/logo-3.png" alt="Streamora" className="w-auto object-contain" style={{ height: '100px' }} />
          </Link>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 relative min-h-screen">
        ...
        {/* Mobile logo */}
        <Link to="/" className="flex items-center mb-8 lg:hidden select-none" aria-label="Streamora home">
          <img src="/logo-3.png" alt="Streamora" className="w-auto object-contain" style={{ height: '100px' }} />
        </Link>
