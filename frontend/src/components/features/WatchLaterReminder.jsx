...
const WatchLaterReminder = () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      featuresService.getWatchLaterReminders().then((r) => r.data.data),
  ...
  const dismissMutation = useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["watch-later-reminders"] }),
  });
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs text-[#606060] ml-auto">
            Saved but not watched
          </span>
        </div>

        <div className="flex gap-3 overflow-x-auto scrollbar-none pb-1">
          {reminders.map((r) => (
            <div key={r._id} className="group relative shrink-0 w-40">
              <Link to={`/watch/${r.video._id}`} className="block">
                ...
                <p className="text-xs text-[#f8f8f8] line-clamp-2 leading-snug">
                  {r.video.title}
                </p>
                {r.video.duration > 0 && (
