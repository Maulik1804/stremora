    ...
    <div ref={boxRef} className={`relative ${className}`}>
      ...
      >
        ...
        >
          <Search
            size={15}
            className={`shrink-0 transition-colors ${open ? "text-[#aaa]" : "text-[#555]"}`}
          />
          ...
          <button
            type="submit"
            className="shrink-0 text-[#666] hover:text-[#f0f0f0] transition-colors p-0.5"
            aria-label="Search"
          >
            <Search size={15} />
          </button>
              ...
              {showSuggestions && (
                <>
                  ...
                  {suggestions.map((item, i) => (
                    ...
                    >
                      {/* Thumbnail */}
                      <div className="w-12 h-8 rounded-lg overflow-hidden bg-[#1a1a1a] shrink-0">
                        {item.thumbnailUrl ? (
                          <img
                          ...
                          <div className="w-full h-full flex items-center justify-center">
                            <Search size={10} className="text-[#444]" />
                          </div>
                        )}
                      </div>
                      {/* Text */}
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-[#e0e0e0] truncate leading-snug">
                          {item.title}
                        </p>
                        {item.owner && (
...
const Navbar = () => {
  ...
  const { data: unreadData } = useQuery({
    ...
    queryFn: () =>
      engagementService.getUnreadCount().then((r) => r.data.data.count),
  ...
  const { data: collabInviteData } = useQuery({
    ...
    queryFn: () =>
      videoService
        ...
        .then((r) => r.data.data.videos?.length ?? 0),
    enabled: isAuthenticated,
    refetchInterval: 30_000,
    staleTime: 20_000,
  });
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 h-14 navbar-blur flex items-center pl-4 pr-6 gap-4">
      {/* Left */}
          ...
          <>
            ...
            <Link to="/collab-invites">
              <motion.button
                ...
                whileTap={{ scale: 0.94 }}
                className="relative p-2 rounded-xl hover:bg-white/6 transition-colors"
                aria-label="Collab Invites"
              >
                <Users size={19} className="text-[#f0f0f0]" />
                {collabInviteCount > 0 && (
                  <span
                    className="absolute -top-0.5 -right-0.5 min-w-4.5 h-4.5 px-1
                                   bg-accent-blue text-white text-[10px] font-bold rounded-full
                                   flex items-center justify-center ring-2 ring-[#080808]"
                  >
            ...
            <div className="relative ml-1 hidden md:block" ref={menuRef}>
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                className="p-1 rounded-full ring-2 ring-transparent hover:ring-white/20 transition-all duration-200"
                  ...
                  >
                    ...
                    <div className="p-1.5">
                      {[
                        ...
                        >
                          <Icon size={15} className="text-[#666] shrink-0" />
                          {label}
                        </Link>
                      ))}
                    </div>
        ...
        {showMobileSearch && (
          ...
          >
            ...
            <motion.button
              ...
              className="p-2 rounded-xl hover:bg-white/6 shrink-0"
              aria-label="Close"
            >
